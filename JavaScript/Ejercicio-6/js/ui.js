...
export function displayBreedDetails(breed){
    ...
    catName.textContent = breed[0]?.name || 'No disponible';
    catTemperament.textContent = breed[0]?.temperament || 'No disponible';
    catTemperament.textContent = breed[0]?.temperament || 'No disponible';
    catLifeSpan.textContent = breed[0]?.life_span || 'No disponible';
    catDescription.textContent = breed[0]?.description || 'No disponible';
    childFriendly.textContent = breed[0]?.child_friendly || 'No disponible';
    catFriendly.textContent = breed[0]?.dog_friendly || 'No disponible';
    origin.textContent = breed[0]?.origin || 'No disponible';
    wikiLink.href = breed[0]?.wikipedia_url || '#';
}